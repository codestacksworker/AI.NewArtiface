...
using SING.Data.Controls.Video.VideoSdkHelper.Struct;
...
namespace SING.Data.Controls.Video.VideoSdkHelper
{
    public class VideoClient
    {
        ...
        public delegate void NetEventNotifyCallBack(int handle, Enum.NET_EVENT_TYPE eNetEventType, IntPtr pUserData);
        ...
        public delegate void AlarmEventNotifyCallBack(
            IntPtr handle, long nAlarmID, Enum.EnumSysAlarmSourceType eAlarmSourceType, IntPtr sAlarmSourceID,
            Enum.EnumSysAlarmType eSysAlarmType, Enum.EnumSysAlarmLevelType eSysAlarmLevelType,
            Enum.EnumSysAlarmHandleType eSysAlarmHandleType, IntPtr sLinkedCameraID, IntPtr sAlarmTime, int nDispIndex,
            IntPtr pUserData);
        ...
        public delegate void DeviceStatusNotifyCallBack(
            int handle, IntPtr szNodeId, Enum.NODE_STATUS eNodeStatus, IntPtr pUserData);
        ...
        public delegate void NodeInfoCallBack(
            int nTotalNodeNum, IntPtr szNodeId, IntPtr sNodeName, Enum.NODE_TYPE eNodeType, Enum.NODE_STATUS eNodeStatus,IntPtr sPrivilege, IntPtr pUserData);
        ...
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_PlayRealVideo", CallingConvention = CallingConvention.Cdecl)]
        public static extern int Video_SDK_PlayRealVideo(int handle, IntPtr hWnd, string szNodeID, int WaitOutTime = 2000, Enum.PLAY_STREAM_TYPE StreamType = Enum.PLAY_STREAM_TYPE.STREAM__FIRST);
        ...
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_CloseVideo", CallingConvention = CallingConvention.Cdecl)]
        ...
        public static extern int Video_SDK_PTZControl(int handle, string szNodeID, Enum.EnumCtrlType ePTZControl,
            Enum.EnumPTZCommandType ePTZCommandType, int nCommandParam);
        ...
        public static extern int Video_SDK_SetPTZPreset(int handle, string szNodeID, Enum.EnumSysPTZPreset eSysPTZPreset,
        ...
        public delegate void GetPTZPresetCallBack(
            int nPresetIndex, IntPtr sPresetName, Enum.EnumSysPresetProperty ePresetProperty, IntPtr pUserData);
        ...
        public delegate void RecordQueryInfoCallBack(
            IntPtr szNodeID, Enum.EnumStorageType eStorageType, Enum.EnumMediaType eMediaType, Enum.EnumRecordType eRecordType,
            Enum.EnumTimeLock eLock, uint uiFileSize, IntPtr sTimeStart, IntPtr sTimeEnd, IntPtr sLockTimeStart,
        ...
        public static extern int Video_SDK_RecordQuery(int handle, string szNodeID, Enum.NODE_TYPE eNodeType,
            Enum.EnumStorageType eStorageType, Enum.EnumMediaType eMediaType, Enum.EnumRecordType eRecordType, string sTimeStart,
            string sTimeEnd, Enum.EnumTimeLock eLock, Enum.EnumSortWay eSort, ulong uiStartRowIndex, int PageSize,
        ...
        public static extern int Video_SDK_PlayRecordVideo(int handle, string szNodeID,
            Enum.EnumRecordOperate eOperate, ref string sFileID, Enum.EnumStorageType eStorageType, IntPtr hWnd);
        ...
        Enum.EnumStorageType eStorage, Enum.EnumRecordType eRecordType, IntPtr hWnd, int nPlatID = 0);
        ...
        public static extern int Video_SDK_RecordPlayCtrl(int handle, IntPtr hWnd, Enum.EnumRecordPlayCtrl ePlayCtrl, int lParam, int wParam);
        ...
        /// 成功返回VIDEO_SDK_NOERROR
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_GetPlayProgress", CallingConvention = CallingConvention.Cdecl)]
        public static extern int Video_SDK_GetPlayProgress(int handle, IntPtr hWnd, ref int nPos);

        #region  播放进度
        //获取录像播放 时间戳
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_GetPlayTimeStamp", CallingConvention = CallingConvention.Cdecl)]
        public static extern int Video_SDK_GetPlayTimeStamp(int handle,IntPtr hWnd,ref int nTimeStamp);
        ...
        /// 成功返回VIDEO_SDK_NOERROR
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_SetRecordPlayProgressByTime", CallingConvention = CallingConvention.Cdecl)]
        ...
        public static extern int Video_SDK_StartLocalRecord(int handle, IntPtr hWnd, string szFileName,
            string szBelongtoDomainID, Enum.EnumRecordType eRecordType);
        ...
        /// <summary>
        /// 开始录像下载
        /// 下载模式为DOWNLOAD_CLIENT时，所有参数都要填，这种是CS下载端使用，会把下载记录写到数据库中
        /// 下载模式为DOWNLOAD_SERVER时，需要登录句柄，存储类型，下载类型，文件ID，结点ID，文件名，以文件的方式下载录像文件，但不会写下载记录
        /// 下载模式为DOWNLOAD_CALLBACK时，需要登录句柄，存储类型，下载类型，文件ID,结点ID，以回调数据包的方式下载，不会写下载记录
        /// szBelongDomainID可以按原来的方式只传一个域节点的ID，也可以传设备的多个父、祖域ID，不要求一定到根域，中间以*号分隔
	    /// szBelongDomainID传多个ID后，录像查询可以在这多个域节点上面查询，原来录的录像只能在设备的直接父域查询
        /// </summary>
        /// <param name="handle">在成功登陆PMS之后得到的SDK操作句柄</param>
        /// <param name="szFileID">需要操作的文件ID</param>
        /// <param name="szNodeID">需要下载的文件所属的通道节点ID</param>
        /// <param name="eStorageType">录像文件的存储类型，可参考EnumStorageType的定义</param>
        /// <param name="eDownloadMode">文件下载的模式，可参考EnumDownloadMode</param>
        /// <param name="eRecordType">录像文件的类型，可参考EnumRecordType</param>
        /// <param name="szBelongDomainID">所属域</param>
        /// <param name="szFileName">文件名称</param>
        /// <param name="szStartTime">开始时间</param>
        /// <param name="szEndTime">结束时间</param>
        /// <returns>
        /// 成功返回VIDEO_SDK_NOERROR
        /// 失败可能返回：
        /// VIDEO_SDK_NOINIT：SDK还没进行初始化
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_DownloadRecordFile")]
        public static extern int Video_SDK_DownloadRecordFile(int handle, string szFileID, string szNodeID, EnumStorageType eStorageType,
            EnumDownloadMode eDownloadMode, EnumRecordType eRecordType = EnumRecordType.SYS_RECORD_ALL, string szBelongDomainID = "", string szFileName = "", string szStartTime = "", string szEndTime = "");
        ...
        /// VIDEO_SDK_NOINIT：SDK还没进行初始化
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_StopDownload")]
        public static extern int Video_SDK_StopDownload(int handle, string szFileID, string szEndTime = "");
        ...
        /// VIDEO_SDK_NOINIT：SDK还没进行初始化
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_DownloadRecordFileByTime", CallingConvention = CallingConvention.Cdecl)]
        public static extern int Video_SDK_DownloadRecordFileByTime(int handle, string szNodeID, string szStartTime, string szEndTime,
        ...
        /// VIDEO_SDK_NOINIT：SDK还没进行初始化
        /// </returns>
        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_StopDownloadByTime", CallingConvention = CallingConvention.Cdecl)]
        public static extern int Video_SDK_StopDownloadByTime(int handle,string szNodeID, string szStartTime, string szEndTime);

        #region 录像回放、下载进度完成后回调
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate void ProgressNotifyCallBack(int handle, EnumTransferType eTransType, int nPos, IntPtr pData, IntPtr pUserData);

        [DllImport("VideoSDK.dll", EntryPoint = "Video_SDK_SetProgressNotify", CallingConvention = CallingConvention.Cdecl)]
        public static extern int Video_SDK_SetProgressNotify([MarshalAs(UnmanagedType.FunctionPtr)]ProgressNotifyCallBack pProgressNotifyCB, IntPtr pUserData);
        #endregion
        #endregion
        ...
        public static extern int Video_SDK_AlarmConfirm(int handle, int nItemCount, long[] nAlarID,
            Enum.EnumSysAlarmStatusType[] eStatus);
        ...
        public delegate void QueryAlarmInfoCallBack(
            long nAlarmID, int nBelongDomainID, int nRepeatCount, int nShieldTime, Enum.EnumSysAlarmType eSysAlarmType,
            Enum.EnumSysAlarmLevelType eSysAlarmLevelType, Enum.EnumSysAlarmSourceType eSysAlarmSourceType,
            Enum.EnumSysAlarmStatusType eSysAlarmStatusType, IntPtr sAlarmTime, IntPtr sDomainName, IntPtr sDeviceName,
        ...
        public static extern int Video_SDK_QueryAlarmInfoByNode(int handle, string sNodeID, Enum.NODE_TYPE eNodeType,
            Enum.EnumSysAlarmType eSysAlarmType, Enum.EnumSysAlarmLevelType eSysAlarmLevelType,
            Enum.EnumSysAlarmSourceType eSysAlarmSourceType, Enum.EnumSysAlarmStatusType eSysAlarmStatusType, string sTimeStart,
        ...
        public delegate void TransAlarmNotifyCallBack(
            int handle, long nAlarmID, Enum.EnumSysAlarmSourceType eAlarmSourceType, IntPtr sAlarmSourceID,
            Enum.EnumSysAlarmType eSysAlarmType, Enum.EnumSysAlarmLevelType eSysAlarmLevelType,
            Enum.EnumSysAlarmHandleType eSysAlarmHandleType, string sLinkedCameraID, string sAlarmTime, IntPtr pUserData);
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate void SDK_Face_DecCBFun(int nPort, IntPtr pBuf, int nSize, ref Struct.SDK_FRAME_INFO pFrameInfo, Struct.SDK_StruDateTime sdt, int nReserved1, int nReserved2);
        ...
        public static extern int Video_SDK_Face_OpenRealTimeStream(int handle, string szNodeID, Enum.PLAY_STREAM_TYPE StreamType, IntPtr pUserData);
        ...
        public static extern int Video_SDK_Face_CloseRealTimeStream(int handle, string szNodeID, Enum.PLAY_STREAM_TYPE StreamType);
